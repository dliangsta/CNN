...
public class Plate {
     ...
     */
    public Plate(final double[][] values) {
     ...
     */
    public Plate convolve(final Plate mask) {
        checkValidMask(mask);
    ...
    private double convolvePixelIJ(final Plate mask, final int i, final int j) {
        double sum = 0.0;
        for (int k = 0; k < mask.getHeight(); k++) {
            for (int l = 0; l < mask.getWidth(); l++) {
                ...
                if (neighborX < 0 || neighborX >= getHeight() || neighborY < 0 || neighborY >= getWidth()) {
                    continue;
                }
    ...
    private void checkValidMask(final Plate mask) {
     ...
     */
    public Plate applyActivation(final ActivationFunction func) {
    ...
    public double[] as1DArray() {
        ...
        for (int row = 0; row < getHeight(); row++) {
            System.arraycopy(values[row], 0 /* Copy the whole row! */, result, row * getWidth(), getWidth());
        }
        return result;
    }
    ...
    public String toString() {
        ...
        builder.append(String.format("Plate with dimensions %dx%d:\n", getHeight(), getWidth()));
        for (int i = 0; i < getHeight(); i++) {
            for (int j = 0; j < getWidth(); j++) {
