...
import static tools.CommonUtils.*;
import static tools.TensorUtils.*;
...
public class FullyConnectedLayer {
    ...
    private FullyConnectedLayer(final double[][] weights, final ActivationFunction activation) {
    ...
    public double[] computeOutput(final double[] input) {
        if (input.length != lastInput.length - 1) {
            throw new IllegalArgumentException(String.format(
                    "Input length in fully connected layer was %d, should be %d.", input.length, lastInput.length));
        }
    ...
    /**
     * Given the error from the previous layer, update the weights and return the
     * error for this layer.
     */
    public double[] propagateError(final double[] proppedDelta, final double learningRate) {
        if (proppedDelta.length != weights.length) {
            throw new IllegalArgumentException(
                    String.format("Got length %d delta, expected length %d!", proppedDelta.length, weights.length));
        }
        ...
        tensorSubtract(weights, scalarMultiply(learningRate, outerProduct(proppedDelta, lastInput), true /* inline */),
                true /* inline */);
        return delta;
    }
    ...
    public String toString() {
        ...
        builder.append(String.format("Number of inputs: %d (plus a bias)\n", weights[0].length - 1));
    ...
    public static class Builder {
        ...
        public Builder setActivationFunction(final ActivationFunction func) {
            checkNotNull(func, "Fully connected activation function");
            this.func = func;
            return this;
        }

        public Builder setNumInputs(final int numInputs) {
            checkPositive(numInputs, "Number of fully connected inputs", false);
            this.numInputs = numInputs;
            return this;
        }

        public Builder setNumNodes(final int numNodes) {
